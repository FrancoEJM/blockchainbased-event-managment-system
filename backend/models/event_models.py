...
class Eventos(Base):
    ...
    usuario_creador = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_USUARIOS.id_usuario")
    )
...
class EventosDefinicion(Base):
    ...
    id_evento = _sql.Column(
        _sql.Integer,
        _sql.ForeignKey("BLC_EVENTOS_CREACION.id_evento"),
        primary_key=True,
        index=True,
    )
    ...
    privacidad = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_PRIVACIDAD.id_privacidad")
    )
...
class EventosParametros(Base):
    ...
    id_evento = _sql.Column(
        _sql.Integer,
        _sql.ForeignKey("BLC_EVENTOS.id_evento"),
        primary_key=True,
        index=True,
    )
    id_parametro = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_PARAMETROS_DEFINICION.id_parametro")
    )

    parametro = _orm.relationship("EventosDefinicion", back_populates="parametros")
    parametros = _orm.relationship(
        "EventosParametrosDefinicion", back_populates="parametro"
    )
