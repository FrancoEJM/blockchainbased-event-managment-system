...
class EventoUsuario(Base):
    ...
    id_evento = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_EVENTOS_CREACION.id_evento"), index=True
    )
    id_usuario = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_USUARIOS.id_usuario"), default=0, index=True
    )
    genero = _sql.Column(
        _sql.Integer, _sql.ForeignKey("BLC_GENERO.id_genero"), default=0
    )
...
class EventoRecursos(Base):
    ...
    id_usuario = _sql.Column(
        _sql.Integer,
        _sql.ForeignKey("BLC_USUARIOS.id_usuario"),
        primary_key=True,
        index=True,
    )
