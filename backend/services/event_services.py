...
async def create_event_record(event_author: int, db: _orm.Session):
    event_record_obj = event_md.Eventos(usuario_creador=event_author)
...
async def save_event_image_default(id, db: _orm.Session):
    image_obj = event_md.EventosImagenes(
        id_evento=id, nombre="default.jpg", path="/data/eventImages/default.jpg"
    )
    db.add(image_obj)
    db.commit()
    db.refresh(image_obj)
...
async def save_event_guest(
    id: int, name: str, email: str, gender, birthdate, db: _orm.Session
):
    existing_user = (
        db.query(user_md.Usuario)
        .filter(user_md.Usuario.correo_electronico == email)
...
async def get_events(user_id: int, db: _orm.Session):
    ...
    invited_events_subquery = (
        db.query(event_user_md.EventoInvitados.id_evento)
        .filter(event_user_md.EventoInvitados.id_usuario == user_id)
        .subquery()
    )
...
async def get_user_events(user_id: int, db: _orm.Session) -> _typing.List[dict]:
    events = (
        db.query(
            ...
            event_md.Eventos.fecha_ejecucion,
            event_md.Eventos.fecha_finalizacion,
        )
    ...
    for event_def, fecha_ejecucion, fecha_finalizacion in events:
        ...
        event_dict["fecha_ejecucion"] = (
            str(fecha_ejecucion) if fecha_ejecucion else None
        )
        event_dict["fecha_finalizacion"] = (
            str(fecha_finalizacion) if fecha_finalizacion else None
        )
...
async def get_event(id, db: _orm.Session):
    return (
        ...
        .filter(event_md.EventosDefinicion.id_evento == id)
        .first()
    )


async def delete_event_invitados(event_id: int, db: _orm.Session):
    db.query(event_user_md.EventoInvitados).filter(
        event_user_md.EventoInvitados.id_evento == event_id
    ).delete()


async def delete_event_imagen(event_id: int, db: _orm.Session):
    db.query(event_md.EventosImagenes).filter(
        event_md.EventosImagenes.id_evento == event_id
    ).delete()


async def delete_event_usuario(event_id: int, db: _orm.Session):
    db.query(event_user_md.EventoUsuario).filter(
        event_user_md.EventoUsuario.id_evento == event_id
    ).delete()


async def delete_event(event_id: int, db: _orm.Session):
    db.query(event_md.EventosDefinicion).filter(
        event_md.EventosDefinicion.id_evento == event_id
    ).delete()
...
async def save_qr_database(path: str, event_id: int, db: _orm.Session):
    qr_object = event_md.EventosQRPublicos(
        id_qr=event_id, id_evento=event_id, path=path
    )
    db.add(qr_object)
    try:
        db.commit()
        ...
        raise _fastapi.HTTPException(
            status_code=404,
            detail="El evento especificado no existe para guardar el QR",
        )


def get_event_stats(event_id: int, db: _orm.Session):
    numero_registros = (
        db.query(_sqlalchemy.func.count(event_user_md.EventoUsuario.id_entrada))
    ...
    detalles = (
        ...
        .join(
            user_md.Genero,
            event_user_md.EventoUsuario.genero == user_md.Genero.id_genero,
        )
        .filter(event_user_md.EventoUsuario.id_evento == event_id)
        .all()
    )
...
async def parse_guests_data(
    ...
    for guest_data in guests:
        ...
        gender_mapping = {"M": 1, "F": 2, "O": 3, "D": 0}
